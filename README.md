...
| 🔒 加密 / 解密 | 用登录密码经 PBKDF2 派生 AES-256 密钥，对文件夹内所有文件做 AES-CBC 加密、重命名为 `*.locked`，并写入隐藏清单 `.hope_manifest.json` 记录原始路径；「解密」反向还原。**全程在后台线程分块流式处理**，界面显示「转圈」遮罩与进度（如 `加密中… (3/20)`），不再卡顿。 |
...
5. **移除**：从列表中移除某个文件夹的登记。移除前会**先恢复其文件**——若已加密则自动解密，若已隐藏则自动取消隐藏（解密依赖当前登录密码），恢复成功后再从列表删除。**不会删除磁盘上的任何文件**。
